...
public class Tool_History_Processor
{
    ...
    (
        ...
        History__Tool_Invocation history
    )
    {
    ...
    (
        History<Tool_Invocation, Texture>.Epoch epoch,
        ref bool error
    )
    {
        GL.ActiveTexture(TextureUnit.Texture0);
        GL.BindTexture(TextureTarget.Texture2D, epoch.EPOCH__TEXTURE.TEXTURE_HANDLE);
        foreach(Tool_Invocation invocation in epoch.Records)
        {
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<int>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<uint>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<float>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<double>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<Vector2>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<Vector2i>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    (
        Shader tool__shader,
        IEnumerable<Uniform<Matrix4>> uniforms,
        ref bool error
    )
    {
        if (error) return;
    ...
    {
        int location;
        foreach(Uniform<T> pair in uniforms)
        {
